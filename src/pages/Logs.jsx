...
// Default logs — always shown even when backend has no logs
const DEFAULT_LOGS = [
  { id: "d1", service: "auth-api",      level: "info",    message: "Server started successfully on port 8000",              timestamp: new Date(Date.now() - 1000*60*1).toISOString() },
  { id: "d2", service: "db-replica",    level: "success", message: "MongoDB Atlas connection established",                   timestamp: new Date(Date.now() - 1000*60*3).toISOString() },
  { id: "d3", service: "payment-api",   level: "info",    message: "Health check passed — all systems nominal",              timestamp: new Date(Date.now() - 1000*60*5).toISOString() },
  { id: "d4", service: "user-service",  level: "warn",    message: "Response time exceeded threshold: 287ms (limit 200ms)", timestamp: new Date(Date.now() - 1000*60*8).toISOString() },
  { id: "d5", service: "cdn-edge",      level: "info",    message: "Cache warmed up — 1,240 assets cached",                 timestamp: new Date(Date.now() - 1000*60*10).toISOString() },
  { id: "d6", service: "webhook-worker",level: "error",   message: "Connection timeout after 30000ms",                      timestamp: new Date(Date.now() - 1000*60*12).toISOString() },
  { id: "d7", service: "auth-api",      level: "info",    message: "JWT token validated for user_id: 4821",                 timestamp: new Date(Date.now() - 1000*60*15).toISOString() },
  { id: "d8", service: "payment-api",   level: "success", message: "Payment processed successfully — order #8821",          timestamp: new Date(Date.now() - 1000*60*18).toISOString() },
  { id: "d9", service: "user-service",  level: "debug",   message: "Executing SQL: SELECT * FROM users WHERE id=4821",      timestamp: new Date(Date.now() - 1000*60*20).toISOString() },
  { id:"d10", service: "cdn-edge",      level: "info",    message: "SSL certificate valid for 89 more days",                timestamp: new Date(Date.now() - 1000*60*25).toISOString() },
]
...
export default function Logs() {
  ...
  const loadLogs = useCallback(async () => {
    try {
      ...
      // Keep default logs if API returns empty
      setCounts(statsData || {})
    } catch {
